...
function coverTile(album) {
  const myRating = album.ratings.find(r => r.member_id === member);
  const listened = Boolean(myRating);
  const author = state.members.find(m => m.id === album.member_id);
  const average = albumAverage(album);
  const averageLabel = average === null ? 'pas encore note par le club' : `moyenne ${average.toLocaleString('fr-FR',{maximumFractionDigits:1})}/10`;
  return `<article class="cover-tile ${listened ? 'listened' : 'locked'}" title="${esc(album.title)} - ${esc(album.artist)}">
    <div class="cover-frame">${coverImage(album)}${listened ? `<span class="score-badge">${myRating.score}</span>` : '<span class="lock-badge" aria-label="Pas encore ecoute par toi">🔒</span>'}</div>
    <div class="cover-caption"><strong>${esc(album.title)}</strong><span>${esc(album.artist)}</span><small>${author ? esc(author.name) + ' · ' : ''}${listened ? `ta note ${myRating.score}/10` : 'pas encore ecoute par toi'} · ${averageLabel}</small></div>
  </article>`;
}
