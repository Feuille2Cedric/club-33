...
(async()=>{
 ...
 try{
  ...
  await page.goto('http://127.0.0.1:3334');await page.getByRole('button',{name:'Cédric',exact:true}).waitFor();await page.screenshot({path:'test-artifacts/welcome.png',fullPage:true});await page.getByRole('button',{name:'Cédric',exact:true}).click();
  ...
  await page.screenshot({path:'test-artifacts/week-desktop.png',fullPage:true});
  ...
  await page.screenshot({path:'test-artifacts/history-desktop.png',fullPage:true});
  ...
  await page.setViewportSize({width:390,height:844});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth));await page.screenshot({path:'test-artifacts/week-mobile.png',fullPage:true});
